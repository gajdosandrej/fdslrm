...
#'
#' @description
#' \code{NNMDOOLSE_orth(X, F, V)} calculates non-negative modified double ordinary least squares estimates (NNMDOOLSE) of variance parameters in orthogonal FDSLRM.
#'
...
#' @import CVXR
#'
#' @example R/Examples/example_NNMDOOLSE_orth.R
#'
#' @export
#'
NNMDOOLSE_orth <- function(X, F, V){
        ...
        SX <- MF %*% X %*% t(X) %*% MF
        s <- Variable(l+1)
        p_obj <- Minimize(sum_squares(SX - (s[l+1] %*% MF) - (MFV %*% diag(s[1:l]) %*% t(MFV))))
        constr <- list(s >= 0)
